...
def paramEncode(params="", charset="", encodeEqualSign=False, encodeAmpersand=False, urlDecodeInput=True, urlEncodeOutput=True,hashType="", fileInput=False, cipher="", key=None, task=""):
...
# It supports Vigenere, Caesar, Rot.
    ...
    if(not cipher == ""):
        ...
        if(cipher == "rot" or cipher == "rot13"):
            if(not key or cipher == "rot13"):
                key = 13;
            else:
                if(task == "encrypt"):
                    key = int(key)
...
def main():
    ...
    optional.add_argument('-cipher', help='cipher to use', dest='cipher')
    ...
    print('Output: %s' % (paramEncode(params=args.str, charset=args.enc, urlDecodeInput=args.udi, urlEncodeOutput=args.ueo, hashType=args.hash, fileInput=args.fi, key=args.key, cipher=args.cipher, task=args.task)))
    print('')
